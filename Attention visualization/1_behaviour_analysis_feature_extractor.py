...
#train_data_dir = Path("data path")
#validation_data_dir = Path('data path')
#test_data_dir = Path('data path')
...
processed_data = ImageFolder(data_dir, transform=a_transform)
...
chkpt_original = 'path to checkpoint/ckeckpoint_file.ckpt'
...
image_names_feat_original = []

for i in range(total_data_points):
    image_tmp, label_tmp = original_data[i]
    img_path = original_data.data[i][0]
    img_name_tmp = str(img_path).split('/')[-1:][0] # use / for linux and \\ for windows to get image name
    image_names_feat_original.append(img_name_tmp)
...
print(image_names_feat_original)

## Save Feature Vector of Original Date
np.save('/content/gdrive/MyDrive/programs/behavior/explanations_mcevit/original_330.npy', feature_vector_original)
# feature_vector_original = np.load('/content/drive/MyDrive/work_2/features_npy/feature_vector_original.npy')

chkpt_processed = '/content/gdrive/MyDrive/programs/models/vit/Eycbcr_l16_epoch=1-step=900.ckpt'
...
image_names_feat_processed = []

for i in range(total_data_points):
    image_tmp, label_tmp = processed_data[i]

    img_path_pro = processed_data.data[i][0]
    img_name_pro_tmp = str(img_path_pro).split('/')[-1:][0] # use / for linux and \\ for windows to get image name
    image_names_feat_processed.append(img_name_pro_tmp)
...
print(image_names_feat_processed)

### Save Feature Vector of Processed Date
np.save('path to file /file name.npy', feature_vector_processed)

print(image_names_feat_original == image_names_feat_processed)
input_image_names = np.asarray(image_names_feat_original)
print(input_image_names)

### Save Feature Vector of Processed Date
np.save('pathe to file/file name.npy', input_image_names)

reload_image_names= np.load('path to file/file name.npy')
print(reload_image_names)
